...
// 1.1 pthread_create 쓰레드 생성을 위해서 사용한다. 
int pthread_create(pthread_t *thread, const pthread_attr_t *attr, void *(*start_routine)(void *), void *arg);
/*
첫번째 아규먼트인 thread 는 쓰레드가 성공적으로 생성되었을때 생성된 쓰레드를 식별하기 위해서 사용되는 쓰레드 식별자이다. 
두번째 아규먼트인 attr 은 쓰레드 특성을 지정하기 위해서 사용하며, 기본 쓰레드 특성을 이용하고자 할 경우에 NULL을 사용한다. 
세번째 아규먼트인 start_routine는 분기시켜서 실행할 쓰레드 함수이다.
네번째 아규먼는인 arg는 쓰레드 함수의 인자이다.
성공적으로 생성될 경우 0을 리턴한다.
*/

// 1.2 pthread_join
int pthread_join(pthread_t th, void **thread_return);
/*
첫번째 아규먼트 th는 기다릴(join)할 쓰레드 식별자이다.
두번째 아규먼트 thread_return은 쓰레드의 리턴(return) 값이다. thread_return이 NULL이 아닐경우 해당 포인터로 쓰레드 리턴 값을 받아올 수 있다.
*/
...
이 함수는 식별번호th인 쓰레드를 detach 시키는데, detach 되었을경우 해당(detach 된) 쓰레드가 종료될 경우 pthread_join을 호출하지 않더라도 즉시 모든 자원이 해제(free) 된다.
여기에서는 pthread_create 호출후 detach 하는 방법을 설명하고 있는데, pthread_create 호출 시 쓰레드가 detach 되도록 할 수도 있다.
*/

// 1.4 pthread_exit
void pthread_exit(void *retval);    
/*
pthread_exit 는 현재 실행 중인 쓰레드를 종료 시키고자 할 때 사용한다. 
만약 pthread_cleanup_push 가 정의되어 있다면, pthread_exit 가 호출될 경우 cleanup handler 가 호출된다. 
...
pthread_exit(3)가 호출되어서 쓰레드가 종료될 때 pthread_cleanup_push 에 의해서 인스톨된 함수가 호출된다. 
routine이 쓰레드가 종료될 때 호출되는 함수이다. arg는 아규먼트이다.
cleanup handlers 는 주로 자원을 되돌려주거나, mutex 잠금 등의 해제를 위한 용도로 사용된다. 
만약 mutex 영역에서 pthread_exit 가 호출되어 버릴 경우 다른 쓰레드에서 영원히 block 될 수 있기 때문이다. 
또한 malloc 으로 할당 받은 메모리, 열린 파일지정자를 닫기 위해서도 사용한다.
*/
...
그리고 pthread_cleanup_push 와 pthread_cleanup_pop 은 반드시 같은 함수 내의 같은 레벨의 블럭에서 한쌍으로 사용해야 한다.
*/
...
보호하고자 하는 데이터를 다루는 코드영역을 단지 한번에 하나의 쓰레드만 실행가능 하도록 하는 방법으로 공유되는 데이타를 보호한다. 
...
두번째 인자인 attr 를 이용해서 mutex 특성을 변경할 수 있다. 기본 mutex 특성을 이용하기 원한다면 NULL 을 사용하면 된다.
...
/*
조건변수 cond를 통해서 신호가 전달될 때까지 블럭된다. 만약 신호가 전달되지 않는다면 영원히 블럭될 수 도 있다. 
...
그러나 시간체크가 가능해서 abstime시간동안 신호가 도착하지 않는다면 error 를 발생하면서 리턴한다. 이때 리턴값은 ETIMEDOUT 이다.
errno 가 세팅되는게 아닌, 리턴값으로 에러가 넘어오는것에 주의해야 한다.
